...
import com.cxyz.homepage.myAdapter.cell.EmptyCell;
import com.cxyz.homepage.myAdapter.cell.ErrorCell;
import com.cxyz.homepage.myAdapter.cell.LoadMoreCell;
import com.cxyz.homepage.myAdapter.cell.LoadingCell;
...
public class MyRecyleViewAdapter extends CardBaseAdapter {
    ...
    private LoadMoreCell mLoadMoreCell; //LoadMore 是否已显示

    private UserClazzsCell userClazzsCell;//要显示的东西
    //private List<TestTask> mData;
    ...
    public MyRecyleViewAdapter() {
        mEmptyCell = new EmptyCell(mData);
        mErrorCell = new ErrorCell(mData);
        mLoadingCell = new LoadingCell(mData);
        mLoadMoreCell = new LoadMoreCell(mData);
        userClazzsCell = new UserClazzsCell(mData);
    }
...
//    }
    ...
    public void onViewAttachedToWindow(RecyclerView.ViewHolder holder) {
        ...
        if (isStaggeredGridLayout(holder)) {
            if (viewType == ERROR_TYPE || viewType == EMPTY_TYPE || viewType == LOADING_TYPE || viewType == LOAD_MORE_TYPE) {
                ...
                //设置显示整个span params.setFullSpan(true);
            }
        }
    }
    ...
    /**
     * 显示LoadingView * <p>请求数据时调用，数据请求完毕时调用
     * {@link #hideLoading }</p> *
    ...
    /**
     * 显示LoadingView * <p>列表显示LoadingView并保留keepCount个Item</p> *
    ...
    /**
     * 显示错误提示 * <p>当网络请求发生错误，需要在界面给出错误提示时，调用{@link #showError}</p> *
    ...
    /**
     * 显示错误提示 * <p>当网络请求发生错误，需要在界面给出错误提示时，调用{@link #showErrorKeepCount(int)},并保留keepCount 条Item</p> *
    ...
    /**
     * 显示LoadMoreView * <p>当列表滑动到底部时，调用{@link #showLoadMore()} 提示加载更多，加载完数据，调用{@link #hideLoadMore()} * 隐藏LoadMoreView,显示列表数据。</p> *
     */
    ...
    /**
     * 隐藏LoadMoreView * <p>调用{@link #showLoadMore()}之后，加载数据完成，调用{@link #hideLoadMore()}隐藏LoadMoreView</p>
     */
